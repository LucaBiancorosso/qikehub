...
# qikehub — client and site configuration artefacts.
...
profile_apple() { # NAME ID P12
  ...
  local base_id="net.qikehub.${HUB_FQDN}.${name}"
...
<plist version="1.0">
<dict>
  <key>PayloadDisplayName</key><string>qikehub ${HUB_FQDN} (${name})</string>
  ...
  <array>
    <dict>
      ...
      <key>PayloadCertificateFileName</key><string>qikehub-ca.cer</string>
    ...
    <dict>
      ...
      <key>PayloadVersion</key><integer>1</integer>
      <key>PayloadDisplayName</key><string>qikehub VPN</string>
      <key>UserDefinedName</key><string>qikehub ${HUB_FQDN}</string>
...
profile_swanctl() { # NAME ID
  ...
  swan_set_proposals
  cat <<EOF
# qikehub client '${name}' — strongSwan >= 6.0 (swanctl)
# Place files under your swanctl dir (Linux: /etc/swanctl, Homebrew: \$(brew --prefix)/etc/swanctl):
#   qikehub-ca.pem -> x509ca/    ${name}.pem -> x509/    ${name}.p12 -> pkcs12/
# Load (prompts for the PKCS#12 password) and connect:
#   swanctl --load-all && swanctl --initiate --child qikehub
connections {
    qikehub {
        version = 2
...
profile_routeros() { # CERTNAME_HINT
  ...
  printf "# qikehub site '%s' — RouterOS 7 — generated %s\n#\n" "$HOME_NAME" "$(date -u +%FT%TZ)"
  if [[ $HOME_AUTH == cert ]]; then
    cat <<EOF
# BEFORE importing:
#   1. Upload qikehub-ca.pem and ${HOME_NAME}.p12 to Files
#   2. /certificate import file-name=qikehub-ca.pem passphrase=""
#      /certificate import file-name=${HOME_NAME}.p12 passphrase="<p12 password>"
#   3. /certificate print — if the names differ from qikehub-ca.pem_0 / ${HOME_NAME}.p12_0, edit below
EOF
  else
...
EOF
  if [[ $HOME_AUTH == cert ]]; then
    printf '/certificate set [find name="qikehub-ca.pem_0"] trusted=yes\n'
  fi
  cat <<EOF

/ip ipsec profile add name=qikehub enc-algorithm=aes-256 hash-algorithm=sha384 prf-algorithm=sha384 dh-group=ecp384 nat-traversal=yes dpd-interval=30s dpd-maximum-failures=3
/ip ipsec proposal add name=qikehub enc-algorithms=aes-256-gcm auth-algorithms=sha512 pfs-group=ecp384 lifetime=1h
/ip ipsec peer add name=qikehub address=${PUBLIC_IP4}/32 port=${IKE_PORT} exchange-mode=ike2 profile=qikehub send-initial-contact=yes comment="qikehub ${HUB_FQDN}"
/ip ipsec identity add peer=qikehub ${auth_line} my-id="fqdn:${id}" remote-id="fqdn:${HUB_FQDN}" match-by=remote-id generate-policy=no
EOF
  for lan in ${src_list//,/ }; do
    printf '/ip ipsec policy add peer=qikehub tunnel=yes src-address=%s dst-address=%s proposal=qikehub action=encrypt level=unique comment="qikehub"\n' "$lan" "$OVERLAY4"
  done
  cat <<EOF

# keep IPsec flows out of fasttrack, and never NAT traffic heading into the tunnel
/ip firewall filter add chain=forward action=accept ipsec-policy=in,ipsec comment="qikehub: decrypted in" place-before=0
/ip firewall filter add chain=forward action=accept ipsec-policy=out,ipsec comment="qikehub: to be encrypted" place-before=0
/ip firewall nat add chain=srcnat action=accept dst-address=${OVERLAY4} comment="qikehub: no NAT into tunnel" place-before=0
EOF
  if [[ $EGRESS == home ]]; then
    cat <<EOF

# EGRESS=home: clients browse via this router; allow them to resolve DNS here
/ip firewall nat add chain=srcnat action=masquerade src-address=${pool} out-interface-list=WAN comment="qikehub: client egress"
/ip firewall filter add chain=input action=accept ipsec-policy=in,ipsec src-address=${pool} protocol=udp dst-port=53 comment="qikehub: client DNS" place-before=0
/ip firewall filter add chain=input action=accept ipsec-policy=in,ipsec src-address=${pool} protocol=tcp dst-port=53 comment="qikehub: client DNS" place-before=0
EOF
  fi
  cat <<EOF
